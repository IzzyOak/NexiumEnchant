package com.nexiummc.nexiumenchant.handlers.particles;
...
import com.nexiummc.nexiumenchant.handlers.particles.ReflectionUtils.PackageType;
import com.nexiummc.nexiumenchant.handlers.particles.ReflectionUtils.PacketType;
...
public enum ParticleEffect
	{
		 ...
		 * <li>The speed value slightly influences the size of this particle
		 * effect
		 * </ul>
		 */
		 ...
		 * <li>The speed value influences the velocity at which the particle
		 * flies off
		 * </ul>
		 */
		FIREWORKS_SPARK("fireworksSpark"),
		/**
		 * A particle effect which is displayed by swimming entities and arrows
		 * in water:
		 * <ul>
		...
		/**
		 * A particle effect which is displayed by air when close to bedrock and
		 * the in the void:
		 * <ul>
		...
		TOWN_AURA("townaura"),
		/**
		 * A particle effect which is displayed when landing a critical hit and
		 * by arrows:
		 * <ul>
		...
		CRIT("crit"),
		/**
		 * A particle effect which is displayed when landing a hit with an
		 * enchanted weapon:
		 * <ul>
		...
		/**
		 * A particle effect which is displayed by primed tnt, torches,
		 * droppers, dispensers, end portals, brewing stands and monster
		 * spawners:
		 * <ul>
		 ...
		 * <li>The speed value causes the particle to be colored black when set
		 * to 0
		 * </ul>
		 */
		MOB_SPELL("mobSpell"),
		 ...
		 * <li>The speed value causes the particle to be always colored black
		 * when set to 0
		 * </ul>
		 */
		MOB_SPELL_AMBIENT("mobSpellAmbient"),
		/**
		 * A particle effect which is displayed when splash potions or bottles
		 * o' enchanting hit something:
		 * <ul>
		 * <li>It looks like a white swirl
		 * <li>The speed value causes the particle to only move upwards when set
		 * to 0
		 * </ul>
		 */
		SPELL("spell"),
		 ...
		 * <li>The speed value causes the particle to be colored green when set
		 * to 0
		 * </ul>
		 */
		NOTE("note"),
		...
		EXPLODE("explode"),
		/**
		 * A particle effect which is displayed by torches, active furnaces,
		...
		FOOTSTEP("footstep"),
		/**
		 * A particle effect which is displayed by swimming entities, rain
		 * dropping on the ground and shaking wolves:
		 * <ul>
		...
		WAKE("wake"),
		/**
		 * A particle effect which is displayed by fire, minecarts with furnace
		 * and blazes:
		 * <ul>
		...
		CLOUD("cloud"),
		/**
		 * A particle effect which is displayed by redstone ore, powered
		 * redstone, redstone torches and redstone repeaters:
		 * <ul>
		 * <li>It looks like a tiny colored cloud
		 * <li>The speed value causes the particle to be colored red when set to
		 * 0
		 * </ul>
		 */
		RED_DUST("reddust"),
		...
		/**
		 * A particle effect which is displayed by blocks beneath a water
		 * source:
		 * <ul>
		...
		SLIME("slime"),
		/**
		 * A particle effect which is displayed when breeding and taming
		 * animals:
		 * <ul>
		 ...
		 * @param requiresWater
		 *            Indicates whether water is required for this particle
		 *            effect to display properly
		 */
		...
		public void display(float offsetX, float offsetY, float offsetZ,
				...
				throws IllegalArgumentException
			{
				...
				new ParticleEffectPacket(name, offsetX, offsetY, offsetZ,
						speed, amount).sendTo(center, range);
			}
		...
		public void display(float offsetX, float offsetY, float offsetZ,
				...
				throws IllegalArgumentException
			{
				...
				new ParticleEffectPacket(name, offsetX, offsetY, offsetZ,
						speed, amount).sendTo(center, players);
			}

		/**
		 * Displays an icon crack (item break) particle effect which is only
		...
		public static void displayIconCrack(int id, byte data, float offsetX,
			...
			{
				new ParticleEffectPacket("iconcrack_" + id + "_" + data,
						offsetX, offsetY, offsetZ, speed, amount).sendTo(
						center, range);
			}
		...
		public static void displayBlockCrack(int id, byte data, float offsetX,
				...
				double range) throws IllegalArgumentException
			{
				...
				new ParticleEffectPacket("blockcrack_" + id + "_" + data,
						offsetX, offsetY, offsetZ, 0, amount).sendTo(center,
						range);
			}
		...
		public static void displayBlockDust(int id, byte data, float offsetX,
				...
				Location center, double range) throws IllegalArgumentException
			{
				...
				new ParticleEffectPacket("blockdust_" + id + "_" + data,
						offsetX, offsetY, offsetZ, speed, amount).sendTo(
						center, range);
			}
		...
		/**
		 * Represents a particle effect packet with all attributes which is used
		 * for sending packets to the players
		 * <p>
		...
		public static final class ParticleEffectPacket
			{
				 ...
				 *            Name of the effect
				 * @param offsetX
				 *            Maximum distance particles can fly away from the
				 ...
				 *            Amount of particles
				 * @throws IllegalArgumentException
				 *             If the speed is lower than 0 or the amount is
				 *             lower than 1
				 * @see #initialize()
				 */
				public ParticleEffectPacket(String name, float offsetX,
						float offsetY, float offsetZ, float speed, int amount)
						throws IllegalArgumentException
					{
						initialize();
				 ...
				 * <p>
				 * <b>Note:</b> These fields only have to be initialized once,
				 * so it will return if {@link #initialized} is already set to
				 * <code>true</code>
				 * 
				 * @throws VersionIncompatibleException
				 *             if accessed packets, fields or methods differ in
				 *             your bukkit version
				 */
						...
						try
							{
								int version = Integer.parseInt(Character
										.toString(PackageType
												.getServerVersion().charAt(3)));
								Class<?> packetClass = PackageType.MINECRAFT_SERVER
										.getClass(version < 7 ? "Packet63WorldParticles"
												: PacketType.PLAY_OUT_WORLD_PARTICLES
														.getName());
								packetConstructor = ReflectionUtils
										.getConstructor(packetClass);
								getHandle = ReflectionUtils.getMethod(
										"CraftPlayer",
										PackageType.CRAFTBUKKIT_ENTITY,
										"getHandle");
								playerConnection = ReflectionUtils.getField(
										"EntityPlayer",
										PackageType.MINECRAFT_SERVER, false,
										"playerConnection");
				 ...
				 * {@link #playerConnection} and {@link #sendPacket} are
				 * initialized
				 * 
								...
								try
									{
										packet = packetConstructor
												.newInstance();
										ReflectionUtils.setValue(packet, true,
												"a", name);
										ReflectionUtils.setValue(packet, true,
												"b", (float) center.getX());
										ReflectionUtils.setValue(packet, true,
						...
						try
							{
								sendPacket.invoke(playerConnection
										.get(getHandle.invoke(player)), packet);
							} catch (Exception exception)
							{
				 ...
				 *            Center location of the effect
				 * @param range
				 *            Range in which players will receive the packet
				 *            (Maximum range for particles is usually 16, but it
				...
				/**
				 * Represents a runtime exception that is thrown if a bukkit
				 * version is not compatible with this library
				 * <p>
				 * This class is part of the <b>ParticleEffect Library</b> and
				 * follows the same usage conditions
				 * 
				 * @author DarkBlade12
				 * @since 1.5
				 */
				...
				/**
				 * Represents a runtime exception that is thrown if packet
				 * instantiation fails
				 * <p>
				 * This class is part of the <b>ParticleEffect Library</b> and
				 * follows the same usage conditions
