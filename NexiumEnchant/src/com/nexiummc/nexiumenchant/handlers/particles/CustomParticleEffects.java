...
public enum CustomParticleEffects
	{
		...
		HUGE_EXPLOSION("hugeexplosion"), LARGE_EXPLODE("largeexplode"), FIREWORKS_SPARK(
				...
				"happerVillager"), ICONCRACK("iconcrack_"), TILECRACK(
				"tilecrack_");

		private String particleName;

		CustomParticleEffects(String particleName)
			{
		...
		public void sendToPlayer(Player player, Location location,
				...
				int count) throws Exception
			{
				...
				ReflectionUtilities.setValue(packet, "b",
						(float) location.getX());
				ReflectionUtilities.setValue(packet, "c",
						(float) location.getY());
				ReflectionUtilities.setValue(packet, "d",
						(float) location.getZ());
